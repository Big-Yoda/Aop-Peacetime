...
--[[ This part is WIP, Citizen.CreateThread( function()
    while true do
        Citizen.Wait(0)
		local playername = GetPlayerName(source)
		local playerid = GetPlayerServerId(source)
		Citizen.InvokeNative(GetHashKey("ADD_TEXT_ENTRY"), 'FE_THDR_GTAO', '~p~ Change Me - ID: ~w~'..playerid..'~p~ | Name:  ~w~'..playername.."~p~ | AOP:  ~w~"..aop)
	end
end)]]
...
AddEventHandler("yoda:peacetime", function(peacetimestatus)

peacetimeon = peacetimestatus
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
            DrawTxt(x2, y2, 1.0,1.0,0.45, "~p~Teamspeak: ~w~" .. display.ts, 255, 255, 255, 255)
